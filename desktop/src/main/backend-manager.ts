...
import { BACKEND_BASE_URL, BACKEND_HEALTH_PATH, BACKEND_HEALTH_POLL_INTERVAL_MS, BACKEND_HOST, BACKEND_PORT, BACKEND_STARTUP_TIMEOUT_MS, EXTERNAL_BACKEND_URL, IS_DEV, backendProjectRoot } from "./constants";
...
function resolvePythonExecutable(root: string): string {
      ...
      const venvBin = isWin ? path.join(root, ".venv", "Scripts", "python.exe") : path.join(root, ".venv", "bin", "python");
...
export class BackendManager {
      ...
      async start(): Promise<void> {
            ...
            if (EXTERNAL_BACKEND_URL) {
                  log.info(`[backend] using external backend at ${EXTERNAL_BACKEND_URL}`);
                  this.emit("starting", "Using external backend…");
            ...
            const child = spawn(python, ["-m", "uvicorn", "app.main:app", "--host", BACKEND_HOST, "--port", String(BACKEND_PORT)], {
                  cwd: root,
                  env: {
                        ...process.env,
                        PYTHONUNBUFFERED: "1",
            ...
            child.on("error", (err) => {
                  ...
                  this.emit("crashed", `Could not start the Python backend: ${err.message}. Make sure Python 3 is installed and the backend virtualenv is set up (see backend/README.md).`);
            });
      ...
      private waitForHealthy(): Promise<void> {
            return new Promise((resolve, reject) => {
                  ...
                  const healthUrl = new URL(BACKEND_HEALTH_PATH, BACKEND_BASE_URL).toString();

                  const poll = () => {
                        if (!this.process && !EXTERNAL_BACKEND_URL) {
      ...
      async stop(): Promise<void> {
            if (EXTERNAL_BACKEND_URL || !this.process) return;
